...
" Language:	Verilog
" Maintainer:	T.G 
" Last Update:  Thu Apr  2 14:38:43 CEST 2009
...
syn keyword verilogType	       supply0 supply1 tri triand trior tri0 tri1 wire wand wor
syn keyword verilogType	       reg integer real time realtime signed unsigned 
...
syn keyword verilogStatement   table task tran
syn keyword verilogStatement   tranif0 tranif1 
syn keyword verilogStatement   trireg use vectored wait
syn keyword verilogStatement   weak0 weak1 xnor xor
...
syn match   verilogNumber "\(\<\d\+\|\)'[sS]\?\s*[bB]\s*[0-1_xXzZ?]\+\>"
syn match   verilogNumber "\(\<\d\+\|\)'[sS]\?\s*[oO]\s*[0-7_xXzZ?]\+\>"
syn match   verilogNumber "\(\<\d\+\|\)'[sS]\?\s*[dD]\s*[0-9_xXzZ?]\+\>"
syn match   verilogNumber "\(\<\d\+\|\)'[sS]\?\s*[hH]\s*[0-9a-fA-F_xXzZ?]\+\>"
...
syn match   verilogDirective   "//\s*synthesis\>.*$"
syn match   verilogDirective   "//\s*pragma\>.*$"
...
syn match verilogFunction "\$[a-zA-Z0-9_]\+\>"
...
if version >= 508 || !exists("did_verilog_syn_inits")
   ...
   HiLink verilogFunction        Function
   HiLink verilogMacro           none
